var go = function () {
    ...
    var loadData = function () {
        ...
        var result;
        var result1Temp = $('#result1').switchbutton('options');
        if (result1Temp.checked) {
            result = '1';
        }
        var result2Temp = $('#result2').switchbutton('options');
        if (result2Temp.checked) {
            result += '|0|2';
        }
        var result3Temp = $('#result3').switchbutton('options');
        if (result3Temp.checked) {
            result += '|3';
        }
        //对首字母是|进行处理
        var firstTemp = result.charAt(0);
        if (firstTemp == '|') {
        ...
        Ajax(true, global.xg["GetXGRoadResultPageList"], dataTemp, function (data) {
            if (!data.Success) {
    ...
    var createResult = function () {
        ...
        Ajax(true, global.xg["GetXGRoadResult"], dataTemp, function (data) {
            if (!data.Success) {
                $.messager.alert('提示', data.Message, 'error');
                return;
            }
    ...
    var clearData = function () {
        ...
        $('#user_name1').val('');
        $('#result1').switchbutton('check');
        $('#result2').switchbutton('uncheck');
        $('#result3').switchbutton('uncheck');
    };
    loadData();
    //加载线路
    ...
    var fileConfirmClick = function () {
        ...
        if (display == 'block') {
            ...
            var url = RTURL + global.export["ExportXGRoadResultPageFile"] + "?LoginID=" + LocalID + "&user_name=" + user_name + "&road_id=" + road_id + "&window_name=" + window_name + "&BeginTime=" + BeginTime + "&EndTime=" + EndTime + "&result=" + result + "&di=" + di + "&columname=" + myData + "&exporttype=" + kinds + "&sortName=" + yilai + "&sortOrder=" + (paixu == "asc" ? false : true) + "&pageSize=" + "100000" + "&pageNum=" + "1";
            XCommon.ClosWating();
            window.open(url);
    ...
    var fileExport = function () {
        $('#yilai').combobox('setValue', 'depart_name');

...
var getAreaData = function () {
    Ajax(true, sys.GetSysAreaList, { LoginID: LocalID }, function (data) {
        if (!data.Success) {
            alert(data.Message);
            return;
        };
        var temp = { 'id': '', 'areaname': '全部' };
        data.Data.push(temp);
        $('#area1').combobox({
            data: data.Data,
            valueField: 'id',
...
var cardRecycle = function () {
    ...
    if (card_id == '' || card_id == null) {
        $.messager.alert("提示", "<p class='infoReport'>未输入8位卡号</p>", "error");
        return;
    }
    ...
    Ajax(true, mj["RecycleCardMjCardUsingSim"], { LoginID: LocalID, card_id: card_id }, function (data) {
        if (!data.Success) {
            alert(data.Message);
            return;
        }
        ...
        $.messager.alert("提示", "<p class='infoReport'>退卡成功</p>", "info");

    });
};

var cardRecycleByPhone = function () {
    var number = $('#hand_tel1').val();
    if (number == '' || number == null) {
        $.messager.alert("提示", "<p class='infoReport'>未输入手机号码</p>", "error");
        return;
    }
    if (!XCommon.CheckMobile(number)) {
        $.messager.alert("提示", "<p class='infoReport'>请输入正确的手机号码</p>", "error");
        return;
    }
    var card_id = parseInt(number).toString(16).toUpperCase();
    if (card_id.length < 8) {
...
var cardMake = function () {
    ...
    if (workStatus != 'kl' && workStatus != 're' && workStatus != "noDevice") {
        $.messager.alert("提示", "<p class='infoReport'>当前卡状态无法制卡</p>", "error");
        return;
    }
    var reg = new RegExp(/^[0-9a-fA-F]{8}$/);
    if (!card_id.match(reg)) {
    ...
    Ajax(true, mj["NewMjCardUsingSim"], { LoginID: LocalID, card_id: card_id, user_file_id: user_file_id, num_incard: num_incard, card_kind: card_kind, begin_date: begin_date, end_date: end_date, work_status: work_status, is_psw: is_psw }, function (data) {
        if (!data.Success) {
            alert(data.Message);
            return;
        }
        ...
        $.messager.alert("提示", "<p class='infoReport'>制卡成功</p>", "info");
    });
};

//手机号制卡
var cardMakeByPhone = function () {
    var number = $('#hand_tel1').val().trim();
    if (!XCommon.CheckMobile(number)) {
        $.messager.alert("提示", "<p class='infoReport'>请输入正确的手机号码", "error");
        return;
    }
    var rowData = tbUser.datagrid('getSelected');
    var temp = $('#id').val();
...
var CardUpdateEndDate = function () {
    ...
    if ((card_id == '' || card_id == null) && (number == '' || number == null)) {
        $.messager.alert("提示", "<p class='infoReport'>未输入卡号或手机号码</p>", "error");
        return;
    }
    if (card_id == '' || card_id == null) {
        if (!XCommon.CheckMobile(number)) {
    ...
    if (workStatus == 'noDevice' || work_status == "lk") {
    ...
    Ajax(true, mj["UpdateMjCardUsingEndDateSim"], { LoginID: LocalID, card_id: card_id, end_date: end_date }, function (data) {
        if (!data.Success) {
            alert(data.Message);
            return;
        }
...
var getDoor = function () {
    Ajax(true, mj["GetMJControlDoorList"], { LoginID: LocalID, "flag": "2" }, function (data) {
        if (!data.Success) {
            alert(data.Message);
            return;
        }; //修改id

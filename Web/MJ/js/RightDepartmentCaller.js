...
var toolAddClick = function () {
    ...
    var treeData = tree.tree('getSelected');
    if (treeData) {
        $('#depart_id').combobox('setValue', treeData.id)
    }
}
...
function getDepartDoor(depart_id) {
    ...
    var temp;
    if (area != '') {
        temp = { LoginID: LocalID, depart_id: depart_id, area_id: area };
    } else {
...
var initializeEvent = function () {
    ...
    $("#save").click(function () {
        ...
        if ($("#depart_id").combobox("getValue").trim() == '') {
        ...
        var result1 = true;
        var allData1 = $("#depart_id").combobox("getData");   //获取combobox所有数据
        for (var i = 0; i < allData1.length; i++) {
            if (depart_id == allData1[i]["id"]) {
                result1 = false;
                break;
            }
        }
        ...
        if (id == null) {
            ...
            $.messager.alert("提示", "<p class='infoReport'>查看状态下不能修改</p>", "error");
            $('#win').window('close');
        }
    });
};

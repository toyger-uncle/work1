...
var toolRemoveClick = function () {
    ...
    $.messager.confirm("提示", "<p class='infoReport'>确定删除选中项数据</p>", function (data) {
        if (data) {                                               //传递参数注意
            Ajax(true, mj["RemoveMjUserDoor"], { LoginID: LocalID, id: rowData.id }, function (data) {
                if (!data.Success) {
                    alert(data.Message);
                    return;
                }
...
function loadDataToWin(rowData) {
    $('#door_id').combobox('setValue', rowData.door_id);
    $('#group_name').combobox('setValue', getGroupID(rowData.group_name));
    $('#apb').combobox('setValue', rowData.apb);
}

var getGroupID = function (group_name) {
    for (var i = 0; i < groupData.length; i++) {
        if (groupData[i].name == group_name) {
            return groupData[i].id;
        }
    }
}
...
var initializeEvent = function () {
    ...
    $("#save").click(function () {
        ...
        if (id === undefined) {
            ...
            Ajax(true, mj["UpdateMjUserDoor"], dataTemp, function (data) {
                if (!data.Success) {
                    alert(data.Message);
                    return;
                }
